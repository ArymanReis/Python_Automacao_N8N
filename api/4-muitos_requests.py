...
def frequencia_nome(name):
    """Obtém um dicionário de estados no formato {id_estado: frequencia}"""
    url = f"https://servicodados.ibge.gov.br/api/v2/censos/nomes/{name}"
    dados_de_frequencia = obter_request(url, params={"groupBy": "UF"}) or []
    return { int(dado["localidade"]): dado["res"][0]["proporcao"] for dado in dados_de_frequencia}
...
def main(name):
    ...
    for id_estado, frequencia in sorted(
        dict_frequencias.items(),
